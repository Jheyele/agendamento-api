...
export class LoginController {
    ...
    async login (request, response){
        ...
        try {
            ...
            const payload = {id: usuario.id, nome: usuario.nome, isAdmin: usuario.is_admin}
            const token = jwt.sign(payload, process.env.SECRET_JWT, {expiresIn: '2h'})
            
            return response.status(200).json({ id: usuario.id, nome: usuario.nome, isAdmin: usuario.is_admin, token: token})
